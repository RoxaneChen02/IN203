...
int main(int nargs, char* argv[])
{
    ...
    int nb_tasks = H;//On demande au processus de calculer une ligne à la fois ?? Ca ralentie pas mal le travaille non ?
    std::vector<int> iters(W * H);
    std::vector<int> recu(W+1);
    //rank = 0 => le processus maitre
    if (rank == 0) {
        ...
        for (int i = 1; i < nbp; ++i) {
            ...
            std::cout << "renvoie à " << i << "la valuer " << count_task << "\n";
            ++count_task;
        }
        
        while (count_task < nb_tasks) { 
            std::cout << "rank 0 recoit qqch \n";
            MPI_Recv(recu.data(), W+1, MPI_INT, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &status); //pk on recoit rien
            count = recu[W];
            std::cout << "recoit de  "<<status.MPI_SOURCE<<" la tache à effectuer count = " << count <<" recu[67] = "<<recu[67] << std::endl;
            for (int i = 0; i < W; ++i) {
                iters[W * (count)+i] = recu[i];
                
            }
            std::cout << "renvoie à " << status.MPI_SOURCE << " la valuer " << count_task<<"\n";
        ...
        count_task = -1;
        for(int i=1;i<nbp;++i){ MPI_Send(&count_task, 1, MPI_INT, i, tag, MPI_COMM_WORLD); 
        std::cout << "renvoie à " << i << " la valuer " << count_task << "\n";
        }
    ...
    if (rank > 0) {
        ...
        while (num_task != -1) {
            ...
            if (num_task >= 0) {
               result = computeMandelbrotSet(W, H, maxIter, num_task, 1);//il faut aussi trouver un moyen d'envoyer num_task
               result[W] = num_task;
               
               MPI_Send(result.data(),W+1, MPI_INT, 0, tag, MPI_COMM_WORLD);
                std::cout <<"recoit la tache = "<< result[W] << " valeur result[67]=  " << result[67] << "\n";
            }
        }
    }
