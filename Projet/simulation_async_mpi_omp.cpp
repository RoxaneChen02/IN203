...
///////
    ...
    // Determine the colour and key based on whether my rank is equal to zero are not.
    ...
    //on suppose que le nombre de processus -1 divise la taux_population
    std::size_t nb_par_proc=contexte.taux_population/(comm_size-1);
    population.reserve(nb_par_proc);
    épidémie::Grille grille{contexte.taux_population};
    épidémie::Grille grille_bis{contexte.taux_population};
    ...
    for (std::size_t i = 0; i < nb_par_proc; ++i )
    {
        std::default_random_engine motor(100*(i+1+my_new_comm_rank*nb_par_proc));
    ...
    int         nombre_immunisés_grippe= (nb_par_proc*23)/100;
    ...
    while (!quitting)
    {
        ...
        if(colour==1){
            ...
            if (jours_écoulés%365 == 0)// Si le premier Octobre (début de l'année pour l'épidémie ;-) )
            {
                ...
                for ( int ipersonne = nombre_immunisés_grippe; ipersonne < int(nb_par_proc); ++ipersonne )
                {
        ...
        màjStatistique(grille, population);
            ...
            //On réuni tous les grilles
        std::vector<épidémie::Grille::StatistiqueParCase> envoie=grille.getStatistiques();
        auto&  reception =grille.getStatistiques();

        MPI_Allreduce(envoie.data(),reception.data(), 3*largeur_grille*hauteur_grille, MPI_INT, MPI_SUM, new_comm);
        ...
        for ( auto& personne : population )
        {
            ...
            ////////////envoie au processus 0 de jours_écoulés//////////////////////////////

            if(my_new_comm_rank==0){
                bool pret;
            
                MPI_Recv(&pret,1, MPI_C_BOOL, 0,0, MPI_COMM_WORLD,&status);

                if(pret)
                {
                MPI_Isend(&jours_écoulés, 1, MPI_INT, 0, 0, MPI_COMM_WORLD,&request);
                MPI_Isend(reception.data(),3*largeur_grille*hauteur_grille,MPI_INT,0,0, MPI_COMM_WORLD,&request);
                }
            }
           ...
           /////////////////////////////////////////////////////////////////////////////////////
            ...
            std::cout 
              <<"Temps passé dans simulation en un pas de temps : " << elapsed_seconds.count()
              << std::flush << std::endl;
            
            total+=elapsed_seconds.count();

            std::cout 
              <<"Temps passé dans simulation moyenne : " << total/jours_écoulés
              << std::flush << std::endl;
        }
        ...
        //#############################################################################################################
            ...
            std::cout 
              <<"Temps passé dans affichage en un pas de temps : " << elapsed_seconds1.count()
              << std::flush << std::endl;
            
            total1+=elapsed_seconds1.count();

            std::cout 
              <<"Temps passé dans affichage moyenne : " << total1/jours
              << std::flush << std::endl;
